...
namespace LogTracer.Core
{
    ...
    public abstract class TraceListenerBase : TraceListener
    {
        ...
        {
            if (Enum.TryParse(Attributes["level"] ?? "All", true, out SourceLevels level) == false)
            {
                throw new ArgumentOutOfRangeException(nameof(level), "level属性值无效,请参考: System.Diagnostics.SourceLevels");
            }

...
$logTracer.error = function (message) {
	$logTracer.popup(message, "error");
};

//触发事件
$logTracer.fireEvent = function (jq, event) {
    $(jq).each(function () {
        if (this.isFireEvent) {
            return;
        }
...
//获取参数
$logTracer.getElementArgs = function (jq, option) {
    try {
...
//克隆对象
$logTracer.clonePlainObject = function (obj) {
    return $.extend({}, obj);
};

//合并对象,数值以obj1为主
$logTracer.mergePlainObject = function (obj1, obj2) {
    var args = $.merge([true, {}], arguments)
    return $.extend.apply($, args);
}

//分析方法参数
$logTracer.parseArgs = function () {
    var args = {
        Number: [],
        String: [],
        Boolean: [],
...
//动态加载文件
$logTracer.include = function (file) {
    var option = $logTracer.parseArgs.apply(window, arguments);
...
//填充table数据
$logTracer.fillTable = function (selector, data, setData) {
    var table = $(selector)
    ...
    for (var i in data) {
        ...
        var html = template.replace(/\{[^{}]+\}/g, function (a) {
            ...
            if (func) {
                var value = func(it, name,i);
                if (value !== undefined) {
                    return $logTracer.IsNull(value);
                }
            }
            return $logTracer.encodeHTML(it[name]);
        });
        tbody.append(html);
    }
};
...
//填充除table以外的循环数据
$logTracer.fillList = function (selector, data, setData) {
    var list = $(selector)
    if (list.length != 1) {
        throw Error("绑定对象太多");
    }
...
//填充不循环的数据
$logTracer.fillData = function (selector, data, setData) {
    var panel = $(selector)
    if (panel.length == 1 && panel.is("table")) {
        $logTracer.fillTable(selector, data, setData);
        return;
    } else if (panel.length == 0) {
        return;
    } else if (panel.length > 1) {
        panel.each(function () {
            $logTracer.fillData(this, data, setData);
        });
        return;
    }
    var func = null;
    ...
    if (data) {
        var html = template.replace(/\{[^{}]+\}/g, function (a) {
            ...
            return $logTracer.encodeHTML(data[name]);
        });
        panel.html(html);
    }
};
...
//格式化json对象或json字符串
$logTracer.jsonFormat = function (json) {
    var obj = json;
...
//获取url参数值
$logTracer.queryString = function (name) {
...
$logTracer.IsNull = function (obj, defaultValue) {
    if (obj != null) {
        return obj;
    }
...
$logTracer.encodeHTML = function (data) {
    if (data == null) {
        return "";
    }
...
$logTracer._api = function (url) {
    ...
    function processResult(result, success, fail) {
        if ("Code" in result && "Message" in result && "Data" in result) {
            if (result.Code == 1) {
                $logTracer.alert(result.Message);
                return;
            }
            if (result.Code == 3) {
            ...
            if (result.Code == 4002) {
                $logTracer.loading();
                if ($logTracer.config.login.url) {
                    $logTracer.config.login.target.location = $logTracer.urlCombine(me.baseUrl, $logTracer.config.login.url);
                } else {
                    $logTracer.popup("用户为登陆", "err");
                }
                return;
            }
            if (result.Code != 0) {
                if (fail) {
                    ...
                    $logTracer.popup(result.Message, "err");
                }
                return;
            }
            result = result.Data;
        ...
        try {
	        ...
	        $logTracer.error("回调方法异常:" + e.message);
        }
    };
    ...
    function ajax(method, route, data, success, fail) {
        ...
        data = data || {};
        data.agent = "ajax";
        data.uk || (data.uk = $logTracer.queryString("uk"));
        data._r_ = Math.random();
        $logTracer.loading();
        return $.ajax({
            dataType: "json",
            url: $logTracer.urlCombine(me.baseUrl, route),
            type: method,
            data: data,
            success: function (r) {
        ...
        setTimeout(function () {
            ...
            if (result.readyState == 1) {
                if (fail) {
                    ...
                    $logTracer.popup("请求超时:" + $logTracer.urlCombine(me.baseUrl, route), "err");
                }
                $logTracer.unLoading();
            }
        }, $logTracer.config.jsonpTimeout);
    }
};

(function () {
    $logTracer.api = new $logTracer._api(window.location.origin || (window.location.protocol + "//" + window.location.host));
})();

$logTracer.api.add = function (name, url) {
    this[name] = new $logTracer._api(url);
};


//处理url
$logTracer.urlCombine = function (url, route) {
    if (route == null) {
        return url;
    }
    if (route.indexOf('://') > 0) {
...
//是否处于loading状态
$logTracer.isLoading = false;

//显示loading框
$logTracer.loading = function () {
    var id = "log-loading";
    if (this.isLoading) {
        return false;
    }
    $logTracer.isLoading = true;
...
jQuery(document).ajaxError(function (event, result, ajax) {
    if (result.responseText && result.responseText.substr(0, 12) == "error_debug:") {
        $logTracer.confirm(result.responseText, "DEBUG");
    } else {
        $logTracer.popup("请求服务器: " + ajax.url + " 失败\n原因: " + result.statusText, "err");
    }
});
